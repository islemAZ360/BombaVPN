...
def send_telegram_receipt_review(req_id, user_email, server_name, receipt_filename, price):
    try:
        ...
        if bot_token and chat_id:
            ...
            caption = f"≡ƒÆ│ *New Payment Receipt!*\n\n" \
                      f"≡ƒæñ *User:* `{user_email}`\n" \
                      f"≡ƒÆ╗ *Server:* {server_name}\n" \
                      f"≡ƒÆ░ *Price:* {price} Γé╜\n\n" \
            ...
            reply_markup = {
                "inline_keyboard": [
                    [
                        {"text": "Γ£à Approve", "callback_data": f"approve_{req_id}"},
                        {"text": "Γ¥î Reject", "callback_data": f"reject_{req_id}"}
                    ]
                ]
            }
...
def login_required(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        try:
            ...
            # ┘à╪¡╪º┘ê┘ä╪⌐ ┘ü┘â ╪¬╪┤┘ü┘è╪▒ ╪º┘ä╪¼┘ä╪│╪⌐ ╪º┘ä┘à╪¡┘ä┘è╪⌐ (┘ä┘ä╪¬╪╖┘ê┘è╪▒ ╪º┘ä┘à╪¡┘ä┘è)
            ...
            if not is_production:
                try:
                    ...
                    data = local_serializer.loads(session_cookie, max_age=5*24*3600)  # 5 ╪ú┘è╪º┘à
...
def add_header(response):
    # ┘à┘å╪╣ ┘â╪▒┘ê┘à ┘à┘å ╪¬╪«╪▓┘è┘å ╪º┘ä╪╡┘ü╪¡╪º╪¬ ╪º┘ä╪»┘è┘å╪º┘à┘è┘â┘è╪⌐ ┘ä╪¬╪¼┘å╪¿ ╪¡┘ä┘é╪⌐ ╪Ñ╪╣╪º╪»╪⌐ ╪º┘ä╪¬╪¡┘à┘è┘ä
...
def session_login():
    ...
    try:
        ...
        # ╪¡┘ä ╪¿╪»┘è┘ä ┘ä┘ä╪¬╪╖┘ê┘è╪▒ ╪º┘ä┘à╪¡┘ä┘è: ╪º┘ä╪¬╪¡┘é┘é ┘à┘å ╪º┘ä┘Ç Token ┘à╪¿╪º╪┤╪▒╪⌐ ┘ê╪Ñ┘å╪┤╪º╪í session cookie ┘è╪»┘ê┘è╪º┘ï
        if not is_production:
            try:
                decoded = firebase_auth.verify_id_token(id_token)
                # ╪Ñ┘å╪┤╪º╪í session cookie ┘à╪«╪╡╪╡ ╪¿╪º╪│╪¬╪«╪»╪º┘à itsdangerous
                from itsdangerous import URLSafeTimedSerializer
                local_serializer = URLSafeTimedSerializer(app.config['SECRET_KEY'])
...
def pay():
    ...
    if request.method == 'POST':
        ...
        if not server_id:
            flash('╪º┘ä╪▒╪¼╪º╪í ╪º╪«╪¬┘è╪º╪▒ ╪º┘ä╪│┘è╪▒┘ü╪▒ / Please select a server', 'error')
            return redirect(request.url)
            
        if 'receipt' not in request.files:
            flash('┘ä┘à ┘è╪¬┘à ╪º╪«╪¬┘è╪º╪▒ ╪╡┘ê╪▒╪⌐ ╪º┘ä┘ê╪╡┘ä', 'error')
            return redirect(request.url)
        ...
        if file and allowed_file(file.filename):
            ...
            # ╪Ñ╪▒╪│╪º┘ä ╪╡┘ê╪▒╪⌐ ╪º┘ä┘ê╪╡┘ä ┘ä╪¬┘è┘ä┘è╪¼╪▒╪º┘à ┘à╪╣ ╪ú╪▓╪▒╪º╪▒ ╪º┘ä┘é╪¿┘ê┘ä ┘ê╪º┘ä╪▒┘ü╪╢
            req_id = doc_ref.id
            try:
            ...
            flash('╪¬┘à ╪Ñ╪▒╪│╪º┘ä ╪º┘ä╪╖┘ä╪¿ ╪¿┘å╪¼╪º╪¡. ╪¼╪º╪▒┘è ┘à╪▒╪º╪¼╪╣╪¬┘ç ┘à┘å ┘é╪¿┘ä ╪º┘ä╪Ñ╪»╪º╪▒╪⌐. / Request submitted successfully. Under review.', 'success')
            return redirect(url_for('user_dashboard'))
...
def add_servers():
    ...
    flash(f'╪¬┘à╪¬ ╪Ñ╪╢╪º┘ü╪⌐ {added} ╪│┘è╪▒┘ü╪▒╪º╪¬ ╪¿┘å╪¼╪º╪¡!', 'success')
    return redirect(url_for('admin_dashboard'))
...
def import_servers():
    ...
    if subscription_text.startswith('http://') or subscription_text.startswith('https://'):
        try:
            ...
            if not is_safe_url(subscription_text):
                flash('╪▒╪º╪¿╪╖ ╪º┘ä╪º╪│╪¬┘è╪▒╪º╪» ╪º┘ä┘à╪▒┘ü┘é ╪║┘è╪▒ ╪ó┘à┘å ╪ú┘ê ┘è╪│╪¬┘ç╪»┘ü ╪╣┘å╪º┘ê┘è┘å ┘à╪¡┘ä┘è╪⌐ ┘à╪¡╪╕┘ê╪▒╪⌐.', 'error')
                return redirect(url_for('admin_dashboard'))
            ...
            if resp.status_code == 200:
                ...
                flash(f'┘ü╪┤┘ä ╪¼┘ä╪¿ ╪º┘ä╪▒╪º╪¿╪╖╪î ╪º┘ä┘â┘ê╪»: {resp.status_code}', 'error')
                return redirect(url_for('admin_dashboard'))
        except Exception as e:
            flash(f'╪¡╪»╪½ ╪«╪╖╪ú ╪ú╪½┘å╪º╪í ╪¼┘ä╪¿ ╪º┘ä╪▒╪º╪¿╪╖: {e}', 'error')
            return redirect(url_for('admin_dashboard'))
    ...
    if found == 0:
        flash('┘ä┘à ┘è╪¬┘à ╪º┘ä╪╣╪½┘ê╪▒ ╪╣┘ä┘ë ╪│┘è╪▒┘ü╪▒╪º╪¬ VLESS ╪╡╪º┘ä╪¡╪⌐ ┘ü┘è ╪º┘ä╪▒╪º╪¿╪╖ ╪ú┘ê ╪º┘ä┘å╪╡.', 'error')
        return redirect(url_for('admin_dashboard'))

    flash(f'╪¬┘à╪¬ ╪Ñ╪╢╪º┘ü╪⌐ {added} ╪│┘è╪▒┘ü╪▒╪º╪¬ VLESS ┘à╪│╪¬┘ê╪▒╪»╪⌐ ╪¿┘å╪¼╪º╪¡!', 'success')
    return redirect(url_for('admin_dashboard'))
...
def edit_server(server_id):
    ...
    if new_name:
        try:
            ...
            flash('╪¬┘à ╪º┘ä╪¬╪╣╪»┘è┘ä ╪¿┘å╪¼╪º╪¡', 'success')
        except Exception as e:
            if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
                return jsonify({'status': 'error', 'message': str(e)}), 400
            flash(f'╪«╪╖╪ú ╪ú╪½┘å╪º╪í ╪º┘ä╪¬╪╣╪»┘è┘ä: ╪º┘ä╪│┘è╪▒┘ü╪▒ ╪║┘è╪▒ ┘à┘ê╪¼┘ê╪» ╪ú┘ê ┘à╪¡╪░┘ê┘ü.', 'error')
    else:
        if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
...
def delete_server(server_id):
    ...
    flash('╪¬┘à ╪¡╪░┘ü ╪º┘ä╪│┘è╪▒┘ü╪▒ ┘ê╪¼╪»┘ê┘ä╪⌐ ┘å┘é┘ä ┘à╪│╪¬╪«╪»┘à┘è┘ç ╪¬┘ä┘é╪º╪ª┘è╪º┘ï', 'success')
    return redirect(url_for('admin_dashboard'))
...
def _approve_request_logic(request_id):
        ...
        if s_expires:
            ...
            if s_expires < expires_at:
                ...
                db.collection('notifications').add({
                    'title': '╪¬┘å╪¿┘è┘ç ╪»┘è┘ê┘å (╪º┘ä╪│┘è╪▒┘ü╪▒ ╪│┘è┘å╪¬┘ç┘è ┘é╪▒┘è╪¿╪º┘ï)',
                    'message': f'╪º┘ä┘à╪│╪¬╪«╪»┘à {user_data.get("email")} ┘ä╪»┘è┘ç ╪º╪┤╪¬╪▒╪º┘â ┘è╪¬╪¼╪º┘ê╪▓ ╪╣┘à╪▒ ╪º┘ä╪│┘è╪▒┘ü╪▒ ╪º┘ä┘à╪«╪¬╪º╪▒. ┘å╪╕╪º┘à ╪º┘ä┘å┘é┘ä ╪º┘ä╪¬┘ä┘é╪º╪ª┘è ╪│┘è╪¬╪»╪«┘ä ┘ä╪º╪¡┘é╪º┘ï ┘ä┘å┘é┘ä┘ç ┘ä╪│┘è╪▒┘ü╪▒ ╪¼╪»┘è╪» ┘ä┘è┘â┘à┘ä ╪º┘ä┘Ç {debt_delta.days} ┘è┘ê┘à ╪º┘ä┘à╪¬╪¿┘é┘è╪⌐.',
                    'created_at': datetime.now(timezone.utc).replace(tzinfo=None),
                    'is_read': False,
                    'type': 'debt'
                })
    ...
    if user_data.get('referred_by'):
        ...
        for r_doc in referrer_docs:
            ...
            if not all_r_subs:
                send_telegram_notification(f"ΓÜá∩╕Å ╪Ñ╪¡╪º┘ä╪⌐ ╪¿╪»┘ê┘å ┘à┘â╪º┘ü╪ú╪⌐!\n╪º┘ä┘à╪│╪¬╪«╪»┘à {user_data.get('email')} ╪º╪┤╪¬╪▒┘ë ╪¿╪º┘é╪⌐ ╪╣╪¿╪▒ ╪▒╪º╪¿╪╖ {r_user.get('email')}╪î ┘ä┘â┘å ╪╡╪º╪¡╪¿ ╪º┘ä╪▒╪º╪¿╪╖ ┘ä╪º ┘è┘à┘ä┘â ╪ú┘è ╪º╪┤╪¬╪▒╪º┘â ╪│╪º╪¿┘é ┘ä┘å╪╢┘è┘ü ┘ä┘ç ╪º┘ä╪ú┘è╪º┘à.")
                continue
            ...
            if active_subs:
                ...
                send_telegram_notification(f"≡ƒÄë ┘à┘â╪º┘ü╪ú╪⌐ ╪Ñ╪¡╪º┘ä╪⌐!\n╪º┘ä┘à╪│╪¬╪«╪»┘à {user_data.get('email')} ╪º╪┤╪¬╪▒┘ë ╪¿╪º┘é╪⌐ ╪╣╪¿╪▒ ╪▒╪º╪¿╪╖ {r_user.get('email')}.\n╪¬┘à╪¬ ╪Ñ╪╢╪º┘ü╪⌐ 7 ╪ú┘è╪º┘à ┘ä╪º╪┤╪¬╪▒╪º┘â┘ç ╪º┘ä┘å╪┤╪╖.")
            else:
                ...
                if expired_subs:
                    ...
                    send_telegram_notification(f"≡ƒÄë ┘à┘â╪º┘ü╪ú╪⌐ ╪Ñ╪¡╪º┘ä╪⌐!\n╪º┘ä┘à╪│╪¬╪«╪»┘à {user_data.get('email')} ╪º╪┤╪¬╪▒┘ë ╪¿╪º┘é╪⌐ ╪╣╪¿╪▒ ╪▒╪º╪¿╪╖ {r_user.get('email')}.\n╪¬┘à ╪¬┘ü╪╣┘è┘ä ╪º╪┤╪¬╪▒╪º┘â┘ç ╪º┘ä┘à┘å╪¬┘ç┘è ┘ä┘à╪»╪⌐ 7 ╪ú┘è╪º┘à ┘à╪¼╪º┘å┘è╪⌐.")
...
def approve_request(request_id):
    ...
    if success:
        flash('╪¬┘à ╪º┘ä┘à┘ê╪º┘ü┘é╪⌐ ╪╣┘ä┘ë ╪º┘ä╪╖┘ä╪¿ ┘ê╪Ñ┘å╪┤╪º╪í ╪º┘ä╪º╪┤╪¬╪▒╪º┘â ╪¿┘å╪¼╪º╪¡ / Request approved successfully', 'success')
    return redirect(url_for('admin_dashboard'))
...
def reject_request(request_id):
    ...
    if success:
        flash('╪¬┘à ╪▒┘ü╪╢ ╪º┘ä╪╖┘ä╪¿ ╪¿┘å╪¼╪º╪¡ / Request rejected successfully', 'success')
    return redirect(url_for('admin_dashboard'))
        ...
        if data.startswith('approve_') or data.startswith('reject_'):
            ...
            try:
                if action == 'approve':
                    ...
                    result_text = "Γ£à Approved by Admin via Telegram" if success else f"Γ¥î Failed: {msg_text}"
                elif action == 'reject':
                    success, msg_text = _reject_request_logic(req_id)
                    result_text = "Γ¥î Rejected by Admin via Telegram" if success else f"Γ¥î Failed: {msg_text}"
...
def delete_user(user_id):
    ...
    flash('╪¬┘à ╪¡╪░┘ü ╪º┘ä┘à╪│╪¬╪«╪»┘à ┘ê╪¼┘à┘è╪╣ ╪¿┘è╪º┘å╪º╪¬┘ç ┘å┘ç╪º╪ª┘è╪º┘ï / User and all data deleted completely', 'success')
    return redirect(url_for('admin_dashboard'))
...
def rescan_servers():
    ...
    flash(f'╪¬┘à ╪¬╪¡╪»┘è╪½ {updated_count} ╪│┘è╪▒┘ü╪▒╪º╪¬ ╪¿┘å╪¼╪º╪¡.', 'success')
    return redirect(url_for('admin_dashboard'))
...
def sync_link(link_id):
    ...
    if not doc.exists:
        return jsonify({'status': 'error', 'message': '╪º┘ä╪▒╪º╪¿╪╖ ╪║┘è╪▒ ┘à┘ê╪¼┘ê╪»'}), 404

    link = doc.to_dict()
    url = link.get('url')
    if not url:
        return jsonify({'status': 'error', 'message': '┘ä╪º ┘è┘ê╪¼╪» ╪▒╪º╪¿╪╖ ┘à╪¡┘ü┘ê╪╕'}), 400

    total_plan_seconds = link.get('total_plan_seconds', 0) or 0
    total_real_seconds = link.get('total_real_seconds', 0) or 0

    if not is_safe_url(url):
        return jsonify({'status': 'error', 'message': '╪º┘ä╪▒╪º╪¿╪╖ ╪║┘è╪▒ ╪ó┘à┘å ╪ú┘ê ┘è╪│╪¬┘ç╪»┘ü ╪┤╪¿┘â╪⌐ ┘à╪¡┘ä┘è╪⌐.'}), 400

    # Re-fetch the subscription content from the saved URL
    try:
        resp = requests.get(url, timeout=10, headers={'User-Agent': 'v2rayNG'})
    except Exception as e:
        return jsonify({'status': 'error', 'message': f'┘ü╪┤┘ä ╪¼┘ä╪¿ ╪º┘ä╪▒╪º╪¿╪╖: {e}'}), 400

    if resp.status_code != 200:
        return jsonify({'status': 'error', 'message': f'┘ü╪┤┘ä ╪¼┘ä╪¿ ╪º┘ä╪▒╪º╪¿╪╖╪î ╪º┘ä┘â┘ê╪»: {resp.status_code}'}), 400
...
def manage_subscription(sub_id):
    ...
    if action == 'cancel':
        ...
        flash('╪¬┘à ╪Ñ┘ä╪║╪º╪í ╪º┘ä╪º╪┤╪¬╪▒╪º┘â ╪¿┘å╪¼╪º╪¡ / Subscription cancelled', 'success')
        
    elif action == 'delete':
        db.collection('subscriptions').document(sub_id).delete()
        flash('╪¬┘à ╪¡╪░┘ü ╪º┘ä╪º╪┤╪¬╪▒╪º┘â ┘å┘ç╪º╪ª┘è╪º┘ï / Subscription deleted permanently', 'success')
        ...
        flash('╪¬┘à ╪¬╪╣╪»┘è┘ä ┘à╪»╪⌐ ╪º┘ä╪º╪┤╪¬╪▒╪º┘â ╪¿┘å╪¼╪º╪¡ / Subscription duration modified', 'success')
        ...
        if server_id:
            ...
            if s_doc.exists:
                ...
                flash('╪¬┘à ╪¬╪╣┘è┘è┘å ╪º┘ä╪│┘è╪▒┘ü╪▒ ┘ä┘ä╪º╪┤╪¬╪▒╪º┘â ╪¿┘å╪¼╪º╪¡ / Server assigned to subscription', 'success')
    ...
    if action == 'assign':
        ...
        if server_id:
            ...
            if s_doc.exists:
                ...
                flash('╪¬┘à ╪¬╪╣┘è┘è┘å ╪º┘ä╪│┘è╪▒┘ü╪▒ ┘â┘à╪┤╪¬╪▒┘â ╪¼╪»┘è╪» ╪¿┘å╪¼╪º╪¡ / New subscription assigned', 'success')
...
def send_admin_message(user_id):
    ...
    flash('╪¬┘à ╪Ñ╪▒╪│╪º┘ä ╪º┘ä╪▒╪│╪º┘ä╪⌐ ┘ä┘ä┘à╪│╪¬╪«╪»┘à / Message sent', 'success')
    return redirect(url_for('admin_dashboard'))
...
def delete_message(message_id):
    ...
    flash('╪¬┘à ╪¡╪░┘ü ╪º┘ä╪▒╪│╪º┘ä╪⌐ ╪¿┘å╪¼╪º╪¡ / Message deleted', 'success')
    return redirect(url_for('admin_dashboard'))
...
def send_message():
    ...
    if text:
        ...
        flash('╪¬┘à ╪Ñ╪▒╪│╪º┘ä ╪▒╪│╪º┘ä╪¬┘â ╪¿┘å╪¼╪º╪¡', 'success')
    return redirect(url_for('user_dashboard'))
...
def get_subscription(token):
    ...
    if not has_active:
        poisoned_link = "vless://00000000-0000-0000-0000-000000000000@127.0.0.1:80?type=tcp&security=none#Γ¥î_All_Subscriptions_Expired_-_Please_Renew"
        combined_links.insert(0, poisoned_link)
        
    if not combined_links:
        poisoned_link = "vless://00000000-0000-0000-0000-000000000000@127.0.0.1:80?type=tcp&security=none#ΓÜá∩╕Å_No_Servers_Found"
                    ...
                    if server_dead or is_temp:
                        ...
                        if candidate_servers:
                            ...
                            if not all_active_servers:
                                ...
                                db.collection('notifications').add({
                                    'title': '╪º╪┤╪¬╪▒╪º┘â ╪¿╪»┘ê┘å ╪│┘è╪▒┘ü╪▒',
                                    'message': f'╪º┘å╪¬┘ç┘ë ╪º┘ä╪│┘è╪▒┘ü╪▒ ╪º┘ä╪«╪º╪╡ ╪¿╪º┘ä┘à╪┤╪¬╪▒┘â {user_email} ┘ê┘ä╪º ╪¬┘ê╪¼╪» ╪│┘è╪▒┘ü╪▒╪º╪¬ ┘å╪┤╪╖╪⌐ ╪¿╪»┘è┘ä╪⌐! ╪¬┘à ┘ü╪╡┘ä ╪º┘ä╪│┘è╪▒┘ü╪▒ ╪╣┘å┘ç ┘à╪ñ┘é╪¬╪º┘ï ┘ä╪¡┘è┘å ╪Ñ╪╢╪º┘ü╪⌐ ╪ú┘ê ╪¬╪¼╪»┘è╪» ╪│┘è╪▒┘ü╪▒.',
                                    'created_at': datetime.now(timezone.utc).replace(tzinfo=None),
                                    'is_read': False,
                                    'type': 'system'
                                })
                        ...
                        if best_server['id'] != allocated_server_id:
                            ...
                            if user_doc.exists:
                                ...
                                if best_exp < exp_at_naive:
                                    ...
                                    db.collection('notifications').add({
                                        'title': '┘å┘é┘ä ╪¬┘ä┘é╪º╪ª┘è ┘à╪╣ ╪»┘è┘å',
                                        'message': f'╪¬┘à ┘å┘é┘ä ╪º┘ä╪º╪┤╪¬╪▒╪º┘â {sub_id} ┘ä┘ä┘à╪│╪¬╪«╪»┘à {user_data.get("email")} ╪Ñ┘ä┘ë ╪│┘è╪▒┘ü╪▒ ╪¼╪»┘è╪» ╪│┘è┘å╪¬┘ç┘è ┘é╪¿┘ä ╪º╪┤╪¬╪▒╪º┘â┘ç! ┘ç┘å╪º┘â ╪»┘è┘å ╪¿┘é┘è┘à╪⌐ {debt_delta.days} ┘è┘ê┘à.',
                                        'created_at': datetime.now(timezone.utc).replace(tzinfo=None),
                                        'is_read': False,
                                        'type': 'debt'
                                    })
            ...
            if now < expires_at <= reminder_threshold:
                ...
                if user_doc.exists:
                    ...
                    if user_email:
                        ...
                        msg['From'] = smtp_email
                        msg['To'] = user_email
                        msg['Subject'] = "╪¬╪░┘â┘è╪▒: ╪º╪┤╪¬╪▒╪º┘â┘â ┘ü┘è GalaxyVPN ┘é╪º╪▒╪¿ ╪╣┘ä┘ë ╪º┘ä╪º┘å╪¬┘ç╪º╪í"
                        
                        body = f"""
                        ╪ú┘ç┘ä╪º┘ï ╪¿┘â╪î
                        
                        ┘å┘ê╪» ╪¬╪░┘â┘è╪▒┘â ╪¿╪ú┘å ╪º╪┤╪¬╪▒╪º┘â┘â ┘ü┘è GalaxyVPN ╪│┘è┘å╪¬┘ç┘è ╪«┘ä╪º┘ä ╪ú┘é┘ä ┘à┘å 48 ╪│╪º╪╣╪⌐.
                        ┘è╪▒╪¼┘ë ╪¬╪¼╪»┘è╪» ╪º┘ä╪º╪┤╪¬╪▒╪º┘â ┘ä╪¬╪¼┘å╪¿ ╪º┘å┘é╪╖╪º╪╣ ╪º┘ä╪«╪»┘à╪⌐.
                        
                        ╪▒╪º╪¿╪╖ ╪º┘ä╪¬╪¼╪»┘è╪»: {request.host_url}pay
                        
                        ┘à╪╣ ╪¬╪¡┘è╪º╪¬ ┘ü╪▒┘è┘é GalaxyVPN.
                        """
...
if __name__ == '__main__':
    ...
    # ┘ü╪¬╪¡ ╪º┘ä┘à╪¬╪╡┘ü╪¡ ╪¬┘ä┘é╪º╪ª┘è╪º┘ï ╪╣┘ä┘ë ╪º┘ä╪▒╪º╪¿╪╖ ╪º┘ä╪╡╪¡┘è╪¡ ╪Ñ╪░╪º ┘â┘å╪º ┘å╪╣┘à┘ä ┘à╪¡┘ä┘è╪º┘ï
